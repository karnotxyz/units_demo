...
async function main() {
  ...
  console.log(
    "Chain ID:",
    Buffer.from(chainId.chain_id.slice(2), "hex").toString(),
  );
  ...
  const balanceResult = await call(unitsAccount, l3TokenAddress, "balanceOf", [
    unitsAccountAddress,
  ]);
  ...
  console.log(
    `Transferring ${transferAmount.toString()} wei to ${recipientAddress}`,
  );
  ...
  console.log(
    ...
    JSON.stringify(transferResult.receipt.finality_status, null, 2),
  );
  ...
  console.log(
    `Withdrawing ${withdrawAmount.toString()} wei to ${starknetAccountAddress}`,
  );
  ...
  console.log(
    "Withdraw receipt status:",
    JSON.stringify(withdrawResult.receipt.finality_status, null, 2),
  );
